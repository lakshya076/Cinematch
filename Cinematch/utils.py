...
def get_rekt(id: int, connection: pymysql.Connection, cursor: pymysql.cursors.Cursor):
    """
    Return recommended movies of a movie in a list, given its `id`
    """
...
def recommend_direct(id: int, depth: int, connection: pymysql.Connection, cursor: pymysql.cursors.Cursor):
    ...
    og_rekt = get_rekt(id, connection, cursor)

    popsorted = pop_sort(og_rekt, connection, cursor)
