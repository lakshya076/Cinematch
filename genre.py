...
class Genre(QDialog, FrameReuse):
    ...
    def __init__(self):
        ...
        for i in range(len(genre_source)):
            self.new_widgets(number=i, title=genre_source[i], scroll_area=self.genre_scroll)
    ...
    def display_frame_color_change(self):
        ...
        self.selected_genre_text.setText(f"Selected genres: {self.genre_flags.count(True)} (Minimum 1)")
    ...
    def done_func(self):
        ...
        if len(genres) < 1:
            ...
            selected_error.error_dialog.setWindowTitle("Genre Selection Error!")
