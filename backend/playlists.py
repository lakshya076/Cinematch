...
def update_password(username: str, name: str, new_pass: str, connection: pymysql.Connection, cursor: pymysql.cursors.Cursor):
    ...
    if status == 1:
        ...
        cursor.execute(f'update playlists set password = "{hashed_password}" where username = "{username}" and name = "{name}"')
        connection.commit()
...
def delete_playlist(username: str, name: str, connection: pymysql.Connection, cursor: pymysql.cursors.Cursor):
    ...
    if data:
        ...
        cursor.execute(f'insert into deleted_playlists values("{data[0]}", "{data[2]}", "{data[3]}", "{data[4]}", "{data[5]}", curdate(), date_add(curdate(), interval 30 day))')
...
def recover_playlist(username: str, name: str, connection: pymysql.Connection, cursor: pymysql.cursors.Cursor):
    ...
    if status == 2:
        ...
        cursor.execute(f'insert into playlists (select username, "user", name, movies, requires_password, password from deleted_playlists where username = "{username}" and name = "{name}")')
