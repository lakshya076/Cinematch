...
title_original = "border-radius:10px; background-color:#F5F5DC;"
...
class Checklist(QDialog, FrameReuse):
    def __init__(self):
        ...
        session = CacheControl(requests.Session(), cache=FileCache('.chk_img_cache'))
    ...
    def done_func(self):
        ...
        if len(movies) < 5:
            ...
            selected_error.error_dialog.setWindowTitle("Movie Selected Error!")
    ...
    def new_widgets(self, number, img, title, scroll_area):
        ...
        _image = QPixmap(image_object)  # converting QImage object to QPixmap object so as to display on the label
        self.movie_image.setPixmap(_image)  # setting the QPixmap object on the image label so as to display the image
